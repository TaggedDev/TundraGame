...
namespace Environment
{
	...
	public static class Noise
	{
		...
		public static float GetNoiseValue(int xCoord, int yCoord, int mapWidth, int mapHeight, int seed, float scale, int octaves,
			float persistance, float lacunarity, Vector2 offset, NormalizeMode normalizeMode)
		{
			float[,] noiseMap = new float[mapWidth, mapHeight];
			...
			if (normalizeMode == NormalizeMode.Local)
				return Mathf.InverseLerp(minLocalNoiseHeight, maxLocalNoiseHeight, noiseMap[xCoord, yCoord]);
			// if normalize mode is global
			float normalizedHeight = (noiseMap[xCoord, yCoord] + 1) / (maxPossibleHeight / 0.9f);
